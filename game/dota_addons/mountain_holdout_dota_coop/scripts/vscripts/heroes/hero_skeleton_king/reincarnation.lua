...
function Reincarnation( event )
	...
	if casterHP == 0 and ability:IsCooldownReady() and casterMana >= abilityManaCost  then
		...
		if caster:GetUnitName() == "npc_dota_creature_boss_skeleton_king_1" then
			CreateUnitByName("npc_dota_creature_boss_skeleton_king_2", respawnPosition, true, nil, nil, DOTA_TEAM_BADGUYS)
		end
		if caster:GetUnitName() == "npc_dota_creature_boss_skeleton_king_2" then
			CreateUnitByName("npc_dota_creature_boss_skeleton_king_3", respawnPosition, true, nil, nil, DOTA_TEAM_BADGUYS)
		end
    	...
    	--Timers:CreateTimer(3, function() grave:RemoveSelf() end)		
    	grave:RemoveSelf()
